...
client = commands.AutoShardedBot(command_prefix=prefix, intents = discord.Intents.all(), activity = discord.Activity(name=f'{prefix}help pour de l\'aide', type=5))
...
async def on_member_join(member):
  ...
  embed=discord.Embed(color=0X800808, title="— Bienvenue !")
...
async def snipe(ctx):
    ...
    try:
        embed = discord.Embed(description=contents , color=0X800808, timestamp=time)
